...
void AdvancedLotPlopUI::SetLotEntries(const std::vector<LotConfigEntry>* entries) {
	...
	for (const auto& old : mruOrdered) {
		auto it = std::find_if(lotEntriesSnapshot.begin(), lotEntriesSnapshot.end(),
		                       [&old](const LotConfigEntry& e) { return e.id == old.id; });
...
void AdvancedLotPlopUI::RenderLotList() {
	...
	if (ImGui::BeginTable("LotTable", 5,
		...
		if (!lotEntriesSnapshot.empty()) {
			...
			std::vector<int> sortOrder = LotConfigTable::BuildSortedIndex(
				temp, favoritesSet, ImGui::TableGetSortSpecs());
			indices.clear();
...
void AdvancedLotPlopUI::ToggleFavorite(uint32_t lotID) {
	if (favoritesSet.count(lotID)) {
		...
		favoritesOrdered.erase(std::remove(favoritesOrdered.begin(), favoritesOrdered.end(), lotID),
		                       favoritesOrdered.end());
	}
	else {
...
void AdvancedLotPlopUI::RegisterPlop(uint32_t lotID) {
	if (lotEntriesSnapshot.empty()) return;
	auto srcIt = std::find_if(lotEntriesSnapshot.begin(), lotEntriesSnapshot.end(),
	                          [lotID](const LotConfigEntry& e) { return e.id == lotID; });
	if (srcIt == lotEntriesSnapshot.end()) {
		LOG_DEBUG("Register plop skipped, lot 0x{:x} not found in entries", lotID);
		return;
	}
	auto existing = std::find_if(mruOrdered.begin(), mruOrdered.end(),
	                             [lotID](const LotConfigEntry& e) { return e.id == lotID; });
